$(function () {
  ...
  $(" .btnArchive ")
    .bind("click", ArchiveOrder)
    .click(function (event) {
      event.preventDefault();
    });
  $(" .btnUnArchive ")
    .bind("click", UnArchiveOrder)
    .click(function (event) {
      event.preventDefault();
    });
  $(" .btnUpdateStatus ")
    .bind("click", UpdateStatus)
    .click(function (event) {
      event.preventDefault();
    });
  $(" .btnUpdatePayment ")
  ...
  $(" #AddRowTemplate ")
    .bind("click", AddRowTemplate)
    .click(function (event) {
      event.preventDefault();
    });

  $(" #SaveTemplate ")
    .bind("click", SaveTemplate)
    .click(function (event) {
      event.preventDefault();
    });
  ...
  $(" #order_tbl ").on(
    "change",
    'input[name^="width"], input[name^="height"], input[name^="qty"]',
    function (event) {
  ...
  $("#order_tbl").on(
    "change",
    'input[name^="up"], input[name^="extra"]',
    function (event) {
      calculateCost($(this).closest("tr"));
      calculateGrandCost();
    }
  ...
  $(" .btnAddLamination").click(function (event) {
    window.location.replace("add_lamination");
  });
  $(" .btnAddQuality").click(function (event) {
    window.location.replace("add_quality");
  });
  $(" .btnAddFinishing").click(function (event) {
...
function calculateSize(row) {
  ...
  row
    .find('input[name^="m2"]')
    .val(((width * height * qty) / 10000).toFixed(2));
}
...
function refreshRowNumber() {
  $(" .order_row ").each(function (i) {
    ...
    $(this)
      ...
      .attr("name", "qty[" + i + "]");
    $(this)
      .find("select[name^='material']")
      .attr("name", "material[" + i + "]");
    $(this)
      .find("select[name^='lamination']")
      .attr("name", "lamination[" + i + "]");
    $(this)
      .find("select[name^='quality']")
      .attr("name", "quality[" + i + "]");
    $(this)
      .find("select[name^='finishing']")
      .attr("name", "finishing[" + i + "]");
    $(this)
      .find("input[name^='up']")
      .attr("name", "up[" + i + "]");
    $(this)
      .find("input[name^='extra']")
      .attr("name", "extra[" + i + "]");
    $(this)
...
function UpdateStatus() {
  ...
  var o_start = par.find(" option[name='opt_process_start'] ").is(":selected")
    ? "1"
    : "0";
  var o_print = par.find(" option[name='opt_process_print'] ").is(":selected")
    ? "1"
    : "0";
  var o_laminate = par
    .find(" option[name='opt_process_laminate'] ")
    .is(":selected")
    ? "1"
    : "0";
  var o_process = par
    .find(" option[name='opt_process_process'] ")
    .is(":selected")
    ? "1"
    : "0";
  var o_check = par.find(" option[name='opt_process_check'] ").is(":selected")
    ? "1"
    : "0";
  var o_ready = par.find(" option[name='opt_process_ready'] ").is(":selected")
    ? "1"
    : "0";
  var o_deliver = par
    .find(" option[name='opt_process_deliver'] ")
    .is(":selected")
    ? "1"
    : "0";
  $.ajax({
    ...
    switch (text) {
      case "1":
        par
          .find(" option[name='opt_process_hold'] ")
          .prop("selected", false)
          .prop("disabled", true);
      ...
      case "2":
        par
          .find(" option[name='opt_process_start'] ")
          .prop("selected", false)
          .prop("disabled", true);
      ...
      case "3":
        par
          .find(" option[name='opt_process_print'] ")
          .prop("selected", false)
          .prop("disabled", true);
        par
          .find(" option[name='opt_process_laminate'] ")
      ...
      case "6":
        par
          .find(" option[name='opt_process_check'] ")
          .prop("selected", false)
          .prop("disabled", true);
      ...
      case "7":
        par
          .find(" option[name='opt_process_ready'] ")
          .prop("selected", false)
          .prop("disabled", true);
        par
          .find(" option[name='opt_process_deliver'] ")
...
function UpdatePayment() {
  ...
  var o_url = par.find(" input[name='url'] ").val();
  var o_id = par.find(" input[name='order_id'] ").val();
  var o_invoiced = par.find(" input[name='cb_invoiced'] ").is(":checked")
    ? "1"
    : "0";
  var o_partial = par.find(" input[name='cb_partial'] ").is(":checked")
    ? "1"
    : "0";
