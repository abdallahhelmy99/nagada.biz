...
class TCPDF_STATIC {
	...
	public static function getObjFilename($type='tmp') {
		$myrandom = substr(base_convert(mt_rand(60466176, 2147483647), 10, 36), 0, 6);
		return tempnam(K_PATH_CACHE, '__tcpdf_'.$type.'_'.md5($myrandom.uniqid('', true).rand().microtime(true)).'_');
	}
	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function getRandomSeed($seed='') {
		...
		$seed .= uniqid('', true);
		$seed .= rand();
		$seed .= substr(base_convert(mt_rand(60466176, 2147483647), 10, 36), 0, 6);
		$seed .= __FILE__;
